...
"""Stage ``schemas/bridge_limits.json`` into ``hephaestus-core`` as package data.
...
the system alone. ``hephaestus.core.registry`` reads it *at import time*, which
makes it a hard packaging dependency: a wheel without it fails on
``import hephaestus.core``, not later at some limits-related call.
...
TARGET = Path("src") / "hephaestus" / "core" / "_data" / "bridge_limits.json"
    ...
    def initialize(self, version: str, build_data: dict[str, Any]) -> None:
        ...
        if source is None:
            ...
            raise RuntimeError(
                ...
                "hephaestus.core imports it at import time; a wheel without it is broken."
            )
