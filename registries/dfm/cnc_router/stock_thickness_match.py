# cnc_router.stock_thickness_match — the design must be cut from stock that exists.
#
# Sheet-goods and plate designs encode their thickness everywhere: tab lengths,
# pocket floors, lamination counts. If the modelled thickness is not a size the
# named material is sold in, every one of those dimensions is wrong by the
# difference. Silent when the part has not claimed a material — a scaffolded
# example plate has no spec, and a finding there would not be a shop question.
#
...
def evaluate(ctx):
    ...
    if material is None:
        if not spec:
            return
        ctx.report(
            f"part.material_spec {spec!r} does not resolve to any materials-registry "
            f"record, so the {measured:.3f} mm modelled thickness cannot be "
    ...
    if not thicknesses:
        ctx.report(
            ...
            f"{measured:.3f} mm modelled part cannot be matched to stock",
            refs=refs,
        ...
        if abs(nearest - measured) > tolerance:
            ...
            ctx.report(
                f"modelled thickness {measured:.3f} mm is not a stock size for "
