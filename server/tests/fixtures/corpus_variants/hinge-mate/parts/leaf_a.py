# Independent second implementation of hinge-mate leaf_a (VALIDATION.md §1:
# every task ships a second solution so its checks provably grade the
# engineering, not the reference geometry back).
#
# Different construction: the knuckle is turned as a finished tube *before* the
# union (the reference bores the assembled leaf), the plate is laid out with
# align= instead of centred positioning, and the plate's two outer corners
# carry a 2 mm corner chamfer the reference does not — in-spec detailing a
# correct shop part may well have, which the acceptance must not punish.
...
plate = Pos(0.0, -hc.plate_standoff, 0.0) * Box(
    hc.knuckle_len, hc.plate_w, hc.plate_t, align=(Align.MAX, Align.MAX, Align.MIN)
)
body = knuckle + plate
# Break the two outer plate corners (the vertical edges at the plate's -Y edge).
_outer_corners = body.edges().filter_by(Axis.Z).group_by(Axis.Y)[0]
...
part.description = "Hinge leaf A (variant build): tube-first knuckle, chamfered corners"
