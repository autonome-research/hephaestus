...
def test_crash_and_conflicting_terminal_project_the_durable_winner(runtime):
    ...
    try:
        ...
        runtime._on_notification("terminal", {
            "run_id": "zero-event-run", "terminal_id": "late-success",
...
def test_late_previous_terminal_cannot_replace_current_ownership(runtime):
    ...
    runtime._on_notification("terminal", {
        "run_id": "old", "terminal_id": "late-old", "state": "failed", "payload": {},
    })
...
def test_cancel_request_is_not_a_terminal_and_capacity_is_reported(runtime):
...
def test_blocking_response_uses_durable_winner_not_late_rpc_status(runtime, monkeypatch):
    ...
    def reply(method, params, **kwargs):
        ...
        rid = params["run_id"]
        runtime._on_notification("terminal", {
            "run_id": rid, "terminal_id": "winner", "state": "failed",
            "payload": {"reason": "backpressure_cancel"},
        })
        runtime._on_notification("terminal", {
            "run_id": rid, "terminal_id": "late", "state": "completed", "payload": {},
        })
...
def test_history_refresh_overlays_only_explicitly_linked_durable_evidence(
    ...
    original = {
        "events": [], "cursor": None, "end_cursor": "opaque", "done": True,
        "user_prompts": [
            {
                "turn": 0, "seq": 0, "text": "go", "run_id": "linked",
