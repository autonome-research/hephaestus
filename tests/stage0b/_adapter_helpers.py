"""Shared builders for the G0B adapter gate tests (not a test module).

Self-contained: builds a minimal real project tree and a deterministic
synthetic :class:`UnpublishedBuild` whose input hashes are internally
consistent, so publication revalidation passes against the live tree it was
derived from. Also importable by the subprocess crash runners (which insert
...
class FakeClock:
    """Injectable manually-advanced clock for retention/GC tests."""
...
def build_for_crash(layout: ProjectLayout, out_dir: Path) -> UnpublishedBuild:
    """The exact build a crash runner and its retrying parent both construct."""
