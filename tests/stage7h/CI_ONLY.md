...
**KNOWN RED (G6's bench clause).** `prior-gates` proves that the *CI job*
`stage gates 1-6` concluded `success`, which covers G6's pytest/ezdxf/pdf/
registry clauses. It does **not** cover G6's numeric clause — "Tier 3 corpus-v1,
Wilson lower-90% ≥ 0.70 on the prose split" (`mission_plan.md` §"G6 status",
still recorded OPEN). That clause is satisfied only by an archived Tier 3 run,
which no pull-request job runs and no check-run conclusion can stand in for. The
latest archived measurement, `bench/results/gpt-5.6-sol/2026-07-29.json`, records
`pass_rate_prose = 0.694` with `meets_gate: false`.

G7H names G6 among the gates that must be green on the release SHA, so this is a
second blocker alongside lane (c): G7H cannot be declared green while
`mission_plan.md` §"G6 status" reads OPEN, regardless of how `prior-gates`
concludes. Delete this paragraph in the same change that closes G6's bench
clause — not before.
